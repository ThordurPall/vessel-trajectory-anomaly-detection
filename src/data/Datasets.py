...
class AISContinuousRepresentation(torch.utils.data.Dataset):
    """A custom Dataset class for processing continuous AIS data samples

    That is, use this class when the AIS attributes should be represented
    using the original input dimensions
    ...
    compute_mean()
        Computs the mean over the inputs
    """
    ...
    def __init__(
        ...
        train_mean : Tensor (Defaults to None)
            Mean of the input features. When train_mean is not
            None, either a validation or test Dataset is created
