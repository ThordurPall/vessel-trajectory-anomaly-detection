...
def train_VRNN(num_epochs, model, train_loader, test_loader, optimizer, scheduler, kl_weight, kl_anneling_start, modelName):
    ...
    kl_weight_step = abs(kl_weight - kl_anneling_start) / (10 * len(train_loader))  # Annealing over 10 epochs.
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i, (_, _, _, lengths, inputs, targets) in enumerate(train_loader):
            ...
            loss, log_px, kl, _ = computeLoss(log_px, log_pz, log_qz, lengths, beta=beta_weight)
        ...
        plotting.make_vae_plots((loss_tot, kl_tot, recon_tot, val_loss_tot, val_kl_tot, val_recon_tot), model, datapoints, testset, testset.params['binedges'], device, figurename=modelName)
        
        print('Epoch {} of {} finished. Trainingloss = {}. Validationloss = {}'.format(epoch, num_epochs, loss_epoch/train_n, val_loss/test_n))
        
        if (epoch%10==0):
            torch.save(model.state_dict(), 'models/saved_models/' + modelName + '_' + str(epoch) + '.pth')
